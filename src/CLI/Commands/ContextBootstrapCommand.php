...
final class ContextBootstrapCommand extends Command
{
    #[\Override]
    ...
    {
        return ($args[0] ?? null) === 'context' && ($args[1] ?? null) === 'bootstrap';
    }
    ...
    public function run(array $args, CommandContext $context): array
    {
        ...
        if ($feature === '') {
            throw new FoundryError(
                'CLI_CONTEXT_BOOTSTRAP_FEATURE_REQUIRED',
                'validation',
                [],
                'Context bootstrap requires a feature slug.',
            );
        }

        $jsonContext = new CommandContext($context->paths()->root(), true);
        $runner = new BatchWorkflowRunner();
        $batch = $runner->run('context bootstrap', [
            [
                'label' => 'context_init',
                'command' => 'context init ' . $feature,
                'run' => static fn(): array => (new ContextInitCommand())->run(['context', 'init', $feature], $jsonContext),
            ],
            [
                'label' => 'inspect_context',
                'command' => 'inspect context ' . $feature,
                'run' => static fn(): array => (new InspectContextCommand())->run(['inspect', 'context', $feature], $jsonContext),
            ],
            [
