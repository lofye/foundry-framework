...
final class DoctorCommand extends Command
{
    ...
    {
        if (in_array('--ready', $args, true)) {
            return $this->runReady($context);
        }
     ...
     */
    private function runReady(CommandContext $context): array
    {
        $jsonContext = new CommandContext($context->paths()->root(), true);
        $batch = (new BatchWorkflowRunner())->run('doctor ready', [
            [
                'label' => 'doctor',
                'command' => 'doctor',
                'run' => fn(): array => $this->run(['doctor'], $jsonContext),
            ],
            [
                'label' => 'compile_graph',
        ...
        return [
            ...
            'message' => $context->expectsJson() ? null : $this->renderReadyMessage($payload),
            'payload' => $context->expectsJson() ? $payload : null,
        ];
    }
     ...
     */
    private function renderReadyMessage(array $payload): string
    {
        $lines = [
            'Doctor ready',
