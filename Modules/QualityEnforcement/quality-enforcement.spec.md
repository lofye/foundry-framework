...
- The coverage wrapper prefers the Valet/Homebrew PHP binary before falling back to the shell `php` found on PATH, unless `PHP_BIN` is explicitly set.
- New app scaffolds include the same coverage wrapper and a `test:coverage` Composer script so scaffolded quality guidance does not depend on the default `php` binary already loading Xdebug.
...
- New app scaffolds include a deterministic `bin/phpunit-coverage` wrapper and `test:coverage` Composer script.
